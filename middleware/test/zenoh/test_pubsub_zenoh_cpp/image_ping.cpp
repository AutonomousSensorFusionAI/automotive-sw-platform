...
const string PUB_KEY = "topic/image_ping";
const string SUB_KEY = "topic/image_pong";
const int PIC = 0; // 0: 5MB, 1: 1MB, 2: 100kB, 3: 10kB
const bool USE_SINGLETON = true;
const double GBE = 1.0; // GbE
const int TEST_COUNT = 1000;
bool FIRST_FLAG = true;
ZenohSession& ZS = ZenohSession::get_instance();
...
public:
  ...
  void data_pub(uint32_t count_)
  {
    if (count_ == 0)
    {
      ...
      thread timeout_thread(&ZenohDataPublisher::check_timeout, this);
  ...
  bool end_flag;
  double duration = 0.0;
  double serial_duration_avg = 0.0;
  double deserial_duration_avg = 0.0;
...
private:
  ...
  void init_image(int pic)
  {
    ...
    // cout << "총 발행 횟수: " << publish_count << endl;
  ...
  vector<uint8_t> serialize_img(uint32_t count_)
  {
  ...
  void data_handler(const string_view &key_expr, const string_view &payload)
  {
    ...
    if (data == 1)  // 초기 발행에 대한 응답
    {
      initial_publish_success = true;
    }
    else if ((data+1) == publish_count) // 마지막 데이터
    {
      last_time = chrono::high_resolution_clock::now();
      duration = chrono::duration<double>(last_time-start_time).count();
      serial_duration_avg = serial_duration_sum / publish_count;
      deserial_duration_avg = deserial_duration_sum / publish_count;
      end_flag = true;
      zenoh_publisher.destroy_zenoh();
  ...
  bool initial_publish_success=false;
...
int main(int argc, char **argv)
{
  ...
  double time_sum = 0.0;
  double ser_sum = 0.0;
  double deser_sum = 0.0;

  for (int i = 0; i < test; i++)
  {
    ZenohDataPublisher zdp(argc, argv);
    
    if (FIRST_FLAG){ // 첫 테스트 시작 flag
      FIRST_FLAG = false;
    }

    if (pub_count == 0)
    {
      pub_count = zdp.publish_count;
    }
    ...
    while(true)
    {
      if (zdp.end_flag && (i != (test-1)))
      {
        time_sum += zdp.duration;
        ser_sum += zdp.serial_duration_avg;
        deser_sum += zdp.deserial_duration_avg;
        break;
      }
      else if (zdp.end_flag && (i == (test-1)))
      {
        time_sum += zdp.duration;
        ser_sum += zdp.serial_duration_avg;
        deser_sum += zdp.deserial_duration_avg;
  ...
  cout << "Test Finished. Publish Count: " << pub_count << endl;
  cout << "Time avg is " << (time_sum / test) << endl;
  cout << "Serial Time avg is " << (ser_sum / test) << endl;
  cout << "Deserial Time avg is " << (deser_sum / test) << endl;
  return 0;
}